...
class Command(NoArgsCommand):
    def handle_noargs(self, **options):
        nowe_bracelety = Bracelet.objects.filter(accepted=0, public=True)
        ...
        nowi_juzerzy = User.objects.filter(date_joined__gte=datetime.date.today() - \
                                           datetime.timedelta(days=7))
        msg_content += u'\r\n\r\nW ciągu ostatniego tygodnia zarejestrowało się ' + \
            str(len(nowi_juzerzy)) + u' nowych użytkowników:'
