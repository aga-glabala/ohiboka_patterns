...
def bracelet(request, bracelet_url, context={}):
    ...
    if not bracelet.public:
        messages.info(request, _("""This bracelet is private. It means you can see it only if
            you have link to this page."""))
    elif bracelet.accepted == 0:
        messages.info(request, _("""This bracelet is not accepted yet. It means you can see it only
             if you have link to this page."""))
    elif bracelet.accepted == -1:
        messages.info(request, _("""This bracelet has been rejected. It means you can see it only
            if you have link to this page."""))
    rates = []
    ...
    return render_to_response('bracelet/bracelet.html', context,
                              RequestContext(request))
...
def addpattern(request):
    ...
    if request.POST['bracelet_id']:
        ...
        b = Bracelet(user=request.user, date=datetime.datetime.today(),
                     url=url, public=public, name=request.POST['name'],
                     accepted=False, difficulty=request.POST['difficulty'],
                     category=BraceletCategory.objects.filter(name=request.POST['category'])[0],
                                            rate=0, type=request.POST['type'])
    b.save()
    index = 0
...
def photos(request, bracelet_id):
    ...
    form = UploadFileForm()
    return render_to_response('bracelet/tabs/photos.html', {'form': form,
                                'bracelet_id': bracelet_id, 'photos': photos,
                                'selectTab': 3}, RequestContext(request))
...
def photo_upload(request, bracelet_id):
    ...
    if form.is_valid():
        handle_uploaded_file(request.FILES['file'],
                             request.POST['bracelet_id'], request.user)
        messages.success(request, _('Photo was uploaded successfully. It will show up here after \
            admin acceptance.'))
        return HttpResponseRedirect('/bracelet/' + bracelet_obj.url +
                                    '#ui-tabs-2')
    ...
    if b.user.email:
        ...
        if status == -1:
            ...
            msg_content = u'Hey! I\'m sorry, but your bracelet was rejected. It was probably to easy or same pattern was already submitted. You can still see this bracelet from your profile' + \
                ...
                u'\r\nAga' + \
                u'\r\nhttp://ohiboka.com'
            EmailMessage(subject, msg_content, 'aga@ohiboka.com', [b.user.email],
                         headers={'Reply-To': ['aga@ohiboka.com']}).send()
        elif status == 1:
